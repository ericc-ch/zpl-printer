...
export function calculateColumnXOffset(columnIndex: number): number {
  return columnIndex * (LABEL_SPECS.LABEL_WIDTH_DOTS + LABEL_SPECS.HORIZONTAL_GAP_DOTS);
}

export function calculatePrintWidth(columns: number): number {
  return (LABEL_SPECS.LABEL_WIDTH_DOTS * columns) + (LABEL_SPECS.HORIZONTAL_GAP_DOTS * (columns - 1));
}
